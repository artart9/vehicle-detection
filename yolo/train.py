"""Train YOLO; promote demo.pt on best val mAP."""
...
from common.metrics import detection_report, evaluate, format_map, yolo_complexity
...
def grad_group(name: str):
    if "pretrained_stem" in name:
        return "backbone"
    if "custom_stack" in name or "sppf" in name:
        return "custom_stack"
...
def main():
    ...
    cfg = loader_settings("yolo", args.runtime)
    epochs = args.epochs
    ...
    model = YOLODetector(
        ...
        backbone_kwargs={"cutoff_layer": "layer3", "trainable_pretrained_layers": 1},
    ).to(device)
    yolo_complexity(model, img_size=img_size, device=device.type)
    ...
    loss_keys = ["loss_cls", "loss_box"]
    grad_groups = ["backbone", "custom_stack", "neck", "head"]
    wdir = weights_dir("yolo")
    ...
    for epoch in range(1, epochs + 1):
        ...
        print("   " + " | ".join(f"{k}: {parts[k]/n_batches:.4f}" for k in loss_keys))
        print(
            "   grad share: "
